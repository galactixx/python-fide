...
class FideEventsClient(FideClientWithPagination):
    """
    A Fide events client to pull all event specific data from the Fide API. Will
    pull data for the latest events as well as detail for a specific event.
    """
    def __init__(self):
    ...
    def get_latest_events(
        ...
        """
        Will return all latest events up to a specific limit. If no limit is provided
        then a limit of 'sys.maxsize' will automatically be set. 
    ...
    def get_event_detail(
        ...
        """
        Given a FideEvent or FideEventID object, will return a FideEventDetail object
        containing further detail for a Fide event. If the ID included does not link to
        a valid Fide event ID, then None is returned.
        
        Args:
            fide_event (FideEvent | FideEventID): A FideEvent or FideEventID object.
        
        Returns:
            FideEventDetail | None: A FideEventDetail object or if the Fide event ID is
                invalid, None.
        """
